...
func (self * SyncCache_t) Create(ts time.Time, key interface{}, value interface{}, evicted Evict) (ok bool) {
	self.mx.Lock()
	ok = self.Cache_t.Create(ts, key, value, evicted)
	self.mx.Unlock()
	return
}

func (self * SyncCache_t) Update(ts time.Time, key interface{}, value interface{}, evicted Evict) (ok bool) {
	self.mx.Lock()
